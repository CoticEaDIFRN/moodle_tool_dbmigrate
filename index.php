...
admin_externalpage_setup('tooldbmigrate');
...
if ($data = $form->get_data()) {
    ...
    try {
        ...
        if ($targetdb->get_tables()) {
            $problem .= get_string('targetdatabasenotempty', 'tool_dbmigrate');
        }
    } catch (moodle_exception $e) {
        $problem .= get_string('notargetconectexception', 'tool_dbmigrate').'<br />'.$e->debuginfo;
    }
    ...
    if ($problem === '') {
        ...
        if ($data->enablemaintenance) {
            ...
            tool_dbmigrate_create_maintenance_file();
        }
        ...
        echo $OUTPUT->heading(get_string('transferringdbto', 'tool_dbmigrate', $data));

        // Do the transfer.
        $CFG->tool_dbransfer_migration_running = true;
        try {
            $feedback = new html_list_progress_trace();
            tool_dbmigrate_transfer_database($DB, $targetdb, $feedback);
            $feedback->finished();
        } catch (Exception $e) {
            if ($data->enablemaintenance) {
                tool_dbmigrate_maintenance_callback();
            }
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('transferdbtoserver', 'tool_dbmigrate'));

$info = format_text(get_string('transferdbintro', 'tool_dbmigrate'), FORMAT_MARKDOWN);
