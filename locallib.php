 ...
 */
function tool_dbmigrate_export_xml_database($description, $mdb) {
 ...
 * @return void
 */
function tool_dbmigrate_transfer_database(moodle_database $sourcedb, moodle_database $targetdb, progress_trace $feedback = null) {
    core_php_time_limit::raise();
    ...
    tool_dbmigrate_rebuild_target_log_actions($targetdb, $feedback);
}
 ...
 */
function tool_dbmigrate_rebuild_target_log_actions(moodle_database $target, progress_trace $feedback = null) {
    global $DB, $CFG;
    require_once("$CFG->libdir/upgradelib.php");

    $feedback->output(get_string('convertinglogdisplay', 'tool_dbmigrate'));
    ...
    $DB = $olddb;
    $feedback->output(get_string('done', 'core_dbmigrate', null), 1);
}
 ...
 */
function tool_dbmigrate_create_maintenance_file() {
    global $CFG;

    core_shutdown_manager::register_function('tool_dbmigrate_maintenance_callback');
    ...
    $message = format_text(get_string('climigrationnotice', 'tool_dbmigrate'), FORMAT_MARKDOWN, $options);
 ...
 */
function tool_dbmigrate_maintenance_callback() {
    global $CFG;
    ...
    if (file_exists("$CFG->dataroot/climaintenance.html")) {
        ...
        error_log('tool_dbmigrate: Interrupted database migration detected, switching off CLI maintenance mode.');
    }
}
